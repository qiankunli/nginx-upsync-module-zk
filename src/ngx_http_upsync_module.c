...
ngx_http_upsync_zk_parse_json(void *data)
{
    ...
    cJSON *nodes = cJSON_GetObjectItem(root, "children");
    if (nodes == NULL) {
        ngx_log_error(NGX_LOG_ERR, ngx_cycle->log, 0,
    ...
    for (server_next = nodes->child; server_next != NULL; 
         server_next = server_next->next) 
    {
        cJSON *temp0 = cJSON_GetObjectItem(server_next, "path");
        ...
        temp0 = cJSON_GetObjectItem(server_next, "data");
...
ngx_http_upsync_zk_parse_init(void *data)
{
    ...
    if (parsed != ngx_strlen(buf)) {
        ngx_log_error(NGX_LOG_ERR, ngx_cycle->log, 0,
                      "upsync_zk_parse_init: parsed body size is wrong");
        return NGX_ERROR;
    }

    ngx_log_debug0(NGX_LOG_DEBUG_HTTP,ngx_cycle->log,0,"ngx_http_upsync_zk_parse_init...");
    if (ngx_strncmp(state.status, "OK", 2) == 0) {
        ngx_log_debug0(NGX_LOG_DEBUG_HTTP,ngx_cycle->log,0,"status OK...");
        if (ngx_strlen(state.http_body) != 0) {
            ctx->body.pos = state.http_body;
            ctx->body.last = state.http_body + ngx_strlen(state.http_body);
            ngx_log_debug1(NGX_LOG_DEBUG_HTTP,ngx_cycle->log,0,"http body %s",ctx->body.pos);
...
ngx_http_upsync_clean_event(void *data)
{
    ...
    if (upsync_type_conf->upsync_type == NGX_HTTP_UPSYNC_CONSUL
        || upsync_type_conf->upsync_type == NGX_HTTP_UPSYNC_ETCD || upsync_type_conf->upsync_type == NGX_HTTP_UPSYNC_ZK)
    {
        if (parser != NULL) {
            ngx_free(parser);
            parser = NULL;
